...
function get_cooccurrence_matrix(posterior::Union{SisPosterior{Int},SimPosterior{Int}})
...
function get_informed_proposal_matrix(posterior::Union{SisPosterior{Int},SimPosterior{Int}}, α::Real)
    C, vmap, vmap_inv = get_cooccurrence_matrix(posterior)
    P = get_cooccurrence_prob_matrix(C, α)
    return cumsum(P), vmap, vmap_inv
end 

function plot_cooccurrence_graph(posterior::Union{SisPosterior{Int},SimPosterior{Int}})
    C, vmap, vmap_inv = get_cooccurrence_matrix(posterior)
    C₀ = copy(C)
    C₀[diagind(C₀)] .= 0
    fig = graphplot(
